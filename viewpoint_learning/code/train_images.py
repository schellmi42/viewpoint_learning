...
from VQDataSet_DLDL import VQDataSet
...
def create_loss_mult_tf(pred_images_all, numOutputs, inImages, ref_views):
    pred_views_list = []
    loss_list = []
    pred_views_ids = []
    for i in range(numOutputs):
        pred_images = tf.reshape(pred_images_all[i], [-1, 32, 32])
        pred_views_id = tf.arg_max(tf.reshape(pred_images, [-1, 1024]), dimension=1)
        pred_views = tf.gather(ref_views, pred_views_id)
        labels = inImages[i]
        loss= create_loss_tf(pred_images, labels)

        pred_views_list.append(pred_views)
        loss_list.append(loss)
        pred_views_ids.append(pred_views_id)
        
    pred_views = tf.concat(pred_views_list, axis = 1)
    loss = tf.stack(loss_list)
    
    lossGraph = tf.reduce_mean(loss)
    
    return lossGraph, pred_views, loss, pred_views_ids
    
def create_loss_tf(pred_images, labels):
    
        return tf.losses.mean_squared_error(labels, pred_images)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='MCRegV_images', help='model (default: MCRegV_small)')
    ...
    inImages = tf.placeholder(tf.float32, [numVQs, None, 32,32], name = 'VQImages')
    inRefViews = tf.placeholder(tf.float32, [1024,3], name = 'refViews')
    ...
    pred_images = model.create_network(inPts, inFeatures, inBatchIds, args.batchSize, 1, args.grow, 3, numVQs, 
        isTraining, keepProbConv, keepProbFull, args.useDropOutConv, args.useDropOut, useRenorm, BNMomentum)
    
    #Create Loss
    lossGraph, pred_views_norm, loss, pred_views_ids = create_loss_mult_tf(pred_images, numVQs, inImages, inRefViews)
    ...
    refViews_np = np.stack((x,y,z), axis=1)
    ...
    for epoch in range(args.maxEpoch):
        ...
        while mTrainDataSet.has_more_batches():
            currbatchSize, pts, features, batchIds, modelList, referenceValuesList, images, vqs3D, invRotationMatrix = mTrainDataSet.get_next_batch()
            ...
            feed_dict[inImages] = images
            feed_dict[inRefViews] = refViews_np
            Pred_views, Pred_views_ids, _, _, step = sess.run([pred_views_norm, pred_views_ids, trainning, accumTrain, global_step], feed_dict)
            ...
            if args.trackVQLoss:
                ...
                Loss = [[] for i in range(numVQs)]
                for i in range(numVQs):
                    for j in range(currbatchSize):
                        pred_vq = images[i][j].reshape(1024)[Pred_views_ids[i][j]]
                        Loss[i].append(1-pred_vq)
                    Loss[i] = np.mean(Loss[i])
            ...
            if lossInfoCounter == 1000/args.batchSize or not mTrainDataSet.has_more_batches():
                prediction, Pred_views_ids = sess.run([pred_images[0], pred_views_ids[0]], feed_dict)
                fig = plt.figure(figsize=[10,5])
                ax=plt.subplot(121)
                im=plt.imshow(prediction[0])
                plt.colorbar(im, ax=ax)
                ax=plt.subplot(122)
                im=plt.imshow(images[0][0])
                plt.colorbar(im, ax=ax)
                plt.savefig('pred.png')
                Pred_vq = images[0][0].reshape(1024)[Pred_views_ids[0]]
        ...
        if epoch%augments_per_epoch==0:
            ...
            while mValDataSet.has_more_batches():
                ...
                currbatchSize, pts, features, batchIds, modelList, referenceValuesList, images, vqs3D, invRotationMatrix = mValDataSet.get_next_batch()
                ...
                feed_dict[inImages] = images
                feed_dict[inRefViews] = refViews_np
                Pred_views, Pred_views_ids, _= sess.run([pred_views_norm, pred_views_ids, accumTest], feed_dict)
    ...
    if not args.no_eval:
        ...
        while mTestDataSet.has_more_batches():
            ...
            currbatchSize, pts, features, batchIds, modelList, referenceValuesList, images, vqs3D, invRotationMatrix = mTestDataSet.get_next_batch()
            feed_dict = create_feed_dict_gauss(inPtHier_tf, [pts, features, batchIds], keepProbConv, 1.0, keepProbFull, 1.0)
            feed_dict[isTraining] = False
            feed_dict[inImages] = images
            feed_dict[inRefViews] = refViews_np
