'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    \brief Code to train viewpoint prediction
...
from VQDataSet_all_VQs import VQDataSet
...
def create_loss_mult_tf(pred_views_all, numOutputs, labels, sizes, t, weightDecay, cosine):
    ...
    for i in range(numOutputs):
        ...
        loss, accuracy = create_loss_tf(pred_views_norm, labels[i], sizes[i], t, cosine)
    ...
    lossGraph = tf.reduce_mean(loss) + regTerm + args.normFactor*tf.reduce_mean(loss_normalizing)
    
    return lossGraph, pred_views_norm, loss, accuracy
...
def logfolder_name(args):
    ...
    if args.symmetric_deformations:
        aug_string += 'symdef_'
    t_string = 't'+str(args.label_threshold)+'_'

    return args.logFolder + '/numPts'+str(args.maxNumPts)+'_'+t_string+'_LR' + str(args.initLearningRate) + '_LDR' + str(args.learningDecayRate) + '_LDF' + str(args.learningDecayFactor)+ '_grow' + str(args.grow)+'_bs'+str(args.batchSize) + '_' + cate_string + normal_string + aug_string + args.affix
...
if __name__ == '__main__':
    ...
    parser.add_argument('--maxNumPts', default = 4096, type = int)
    ...
    parser.add_argument('--single_label', action = 'store_true', help = 'use best view as label')
    ...
    if args.single_label:
        lossGraph, pred_views_norm, loss, accuracy = create_loss_mult_tf(pred_views, numVQs, inLabels, inSizes, t, args.weightDecay, args.cosineLoss)
    else:
        lossGraph, pred_views_norm, loss, accuracy = create_loss_mult_tf(pred_views, numVQs, inLabels_mult, inSizes_mult, t, args.weightDecay, args.cosineLoss)
    ...
    if not args.no_eval:
        ...
        while mValDataSet.has_more_batches():
            ...
            currbatchSize, pts, features, batchIds, labels, labels_mult, signs, modelList, modelParamsList, areas, referenceValuesList, invRotationMatrix, RotationMatrix, vqs3D = mValDataSet.get_next_batch()
            feed_dict = create_feed_dict_mult(inLabels, labels, inLabels_mult, labels_mult, inSizes, inSizes_mult, inPtHier_tf, [pts, features, batchIds], keepProbConv, 1.0, keepProbFull, 1.0, numVQs)
            feed_dict[isTraining] = False
            feed_dict[inSigns] = signs
            Pred_views, _= sess.run([pred_views_norm, accumTest], feed_dict)
        ...
        with open(resultFolder+'/ValDistRes.txt', "a") as myfile:
            myfile.write(str(accumTestDistance) + '\n')
        with open(resultFolder+'/TestLoss.txt', "a") as myfile:
            myfile.write(np.array2string(np.mean(TestLoss,axis=1),precision=2)[1:-1] + '\n')
        with open(resultFolder+'/ValAccuracy.txt', "a") as myfile:
        ...
        while mTestDataSet.has_more_batches():
            ...
            currbatchSize, pts, features, batchIds, labels, labels_mult, signs, modelList, modelParamsList, areas, referenceValuesList, invRotationMatrix, RotationMatrix, vqs3D = mTestDataSet.get_next_batch(repeatModelInBatch=args.tta)
            feed_dict = create_feed_dict_mult(inLabels, labels, inLabels_mult, labels_mult, inSizes, inSizes_mult, inPtHier_tf, [pts, features, batchIds], keepProbConv, 1.0, keepProbFull, 1.0, numVQs)
            feed_dict[isTraining] = False
            feed_dict[inSigns] = signs
            Pred_views, _= sess.run([pred_views_norm, accumTest], feed_dict)
