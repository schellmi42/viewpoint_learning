'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    \brief Code with dataset for GL learning
...
from DataProcessing import generate_labels, load_data_from_disk, fibonacci_sphere
...
class VQDataSet(DataSet):
    ...
    def __init__(self, train, maxStoredNumPoints, batchSize,
        ...
        self.vqs3d_ = [[] for i in range(self.numVQs_)]
        self.areas_ = []
        ...
        for folder in folders:
            ...
            if 'polygonization' in folder: inset = lambda(index): True
            ...
            if os.path.exists(os.path.join(DATA_DIR, folder)+'/small.txt'):
                ...
                for filedir in os.listdir(csv_dir):
    ...
    def get_next_batch(self, repeatModelInBatch = False):
        ...
        accumVQs3d = [ [] for i in range(self.numVQs_)]
        accumSpheres = []
        accumRotationMatrix = []
            ...
            if self.iterator_ < len(self.randomSelection_):
                ...
                for vq_i in range(self.numVQs_):
                    if vq_i == self.uv_index_ or vq_i == self.fv_index_:
                        ...
                        accumVQs3d[vq_i].append(self.vqs3d_[vq_i][currModelIndex])
                ...
                if (self.maxPtsxBatch_ == 0) or ((numPtsInBatch+currModelNumPts) <= self.maxPtsxBatch_):
                    ...
                    if self.augment_:
                        ...
                        pts3d,_,_ = self._augment_data_rot_(pts3d, self.augmentMainAxis_, self.augmentSmallRotations_, rotationMatrix)
                        for vq_i in range(self.numVQs_):
        ...
        return numModelInBatch, accumPts, accumFeatures, accumBatchIds, accumModels, accumReferenceValues, accumInvRotationMatrix, accumRotationMatrix, accumVQs3d, accumSpheres
